            ...
            assign(
                {
                    ...
                    boundVariables: {
                        ...
                        paragraphSpacing:
                            await resolveField("paragraphSpacing"),
                    },
                },
                nameMeta,
            ),
        ...
        for (var j = 0; j < style.paints.length; j++) {
            ...
            if (paint.type === "SOLID") {
                ...
                for (var s = 0; s < gradientStops.length; s++) {
                    ...
                    var stopHex = rgbToHex(
                        stop.color.r,
                        stop.color.g,
                        stop.color.b,
                    );
                    var stopVar =
                        stop.boundVariables && stop.boundVariables.color
                            ? await resolveVariableAlias(
        ...
        for (var v = 0; v < collection.variableIds.length; v++) {
            ...
            for (var m = 0; m < modeIds.length; m++) {
                ...
                for (var mm = 0; mm < collection.modes.length; mm++) {
                    if (collection.modes[mm].modeId === modeId) {
                        modeName = collection.modes[mm].name;
                        break;
                    }
                }
                valuesByMode[modeName] = await resolveValue(
...
async function resolveLibraryVariableValues(libraryVariables) {
    ...
    for (var c = 0; c < libraryVariables.length; c++) {
        ...
        for (var v = 0; v < col.variables.length; v++) {
            ...
            try {
                imported = await figma.variables.importVariableByKeyAsync(
                    descriptor.key,
                );
            } catch (e) {
...
figma.ui.onmessage = async function (msg) {
    ...
    if (msg.type === "EXPORT") {
        try {
            figma.ui.postMessage({
                type: "STATUS",
                message: "Collecting text styles\u2026",
            });
            var textStyles = await extractTextStyles();

            figma.ui.postMessage({
                type: "STATUS",
                message: "Collecting paint styles\u2026",
            });
            var paintStyles = await extractPaintStyles();

            figma.ui.postMessage({
                type: "STATUS",
                message: "Collecting effect styles\u2026",
            });
            var effectStyles = await extractEffectStyles();

            figma.ui.postMessage({
                type: "STATUS",
                message: "Collecting variables\u2026",
            });
            var variables = await extractVariables();

            figma.ui.postMessage({
                type: "STATUS",
                message: "Collecting library variables\u2026",
            });
            ...
            var payload = {
                meta: {
                    ...
                    counts: {
                        ...
                        variables: variables.reduce(function (acc, c) {
                            return acc + c.variables.length;
                        }, 0),
                        libraryCollections: libraryVariables.length,
                        libraryVariables: libraryVariables.reduce(function (
                            acc,
                            c,
                        ) {
                            return acc + c.variables.length;
